(function () {
    // This code exists to support functionality in LocalHomeworkStore.cs. It provides convenient access to
    // the browser's IndexedDB APIs, along with a preconfigured database structure.

    const db = idb.openDB('Homeworks', 1, {
        upgrade(db) {
            db.createObjectStore('homeworks', { keyPath: 'id' });
        },
    });

    window.localHomeworkStore = {
